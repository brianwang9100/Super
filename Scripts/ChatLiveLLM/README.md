...
5. Sends two turns through `ChatSession.send(...)`:
   - **Turn 1**: plain text ("what is the capital of France?")
   - **Turn 2**: tool use ("call `echo` with 'pong from echo' and summarize")
6. Streams `ChatEvent`s to stdout as they arrive.
7. Prints the final persisted message + tool-call rows.
