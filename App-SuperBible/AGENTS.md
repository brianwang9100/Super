...
- **Persona principles + guardrails + eval:** the doctrinal stance (historic-creedal, non-neutral), the *describe-don't-prescribe* contested-topic guardrail, the crisis-handling rule, the response taxonomy, and the Codex model-tier eval harness all live in [`../docs/SuperBible/PERSONA_EVAL.md`](../docs/SuperBible/PERSONA_EVAL.md). When you change persona behavior, use `$superbible-persona-eval` to select a dry run, scoped real run, or explicitly confirmed full matrix over `eval/superbible-persona/corpus.json`, then keep that doc honest. The tool-routing half of the guardrail also lives in the Bible applet briefing (`../Packages/Bible/Sources/Bible/Resources/SystemPrompt.md`) — keep the two consistent.
