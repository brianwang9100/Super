...
Snapshot baselines are pixel-exact comparisons. A baseline recorded on one Xcode + iOS simulator runtime will fail on another, even between minor versions of the same Xcode, because the system text renderer and SwiftUI layout passes change. **Always record against CI's runtime.** Today that is **Xcode 26.3 + iOS 26.2 simulator** on a `macos-15` runner image; verify with `xcodebuild -version` and `xcrun simctl list runtimes` against the latest CI log before recording.

- **Xcode**: pinned by `maxim-lobanov/setup-xcode@v1` in `.github/workflows/ios-build.yml`. Match the version locally — install via `xcodes install <version>` and select with `xcode-select -s /Applications/Xcode.app` (or `DEVELOPER_DIR=...` per-command when multiple Xcodes are installed). The `xcodebuild -version` build number in the worktree and on a fresh CI log must agree.
- **iOS simulator runtime**: CI uses whatever runtime ships with the resolved Xcode's bundled SDK (today: iOS 26.2 SDK). If that runtime is no longer downloadable from Apple (`xcodebuild -downloadPlatform iOS -buildVersion <X> → "not available for download"`), there are two options:
