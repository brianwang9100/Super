...
Snapshot baselines are pixel-exact comparisons. Changes to the Xcode build, simulator runtime build, or iPhone model can change text rendering and layout. The migration target is **Xcode 27 beta 6 (build `27A5252f`) + iOS 27.0 (runtime build `24A5423a`) on iPhone 17**, verified on the `xcode-27` GitHub-hosted preview image. That image runs macOS 26; `xcode-27` is not a `macos-27` runner. See `docs/XCODE_27_PCC_PLAN.md` for migration completion and validation status.

- **Xcode**: workflows select `27.0-beta` and then assert exact `XCODE_BUILD: "27A5252f"` before restoring compiled caches. The selector alone is not a beta pin. Match `xcodebuild -version` locally and prefer per-command `DEVELOPER_DIR` over changing another worktree's global selection. Xcode 27 requires **macOS 26.4 or later**. Do not upgrade/restart the host OS without user authorization.
- **iOS simulator runtime**: `Pick iOS simulator` asserts one available iOS 27.0 runtime at build **`24A5423a`**. Beta builds can share the same `iOS-27-0` identifier, so `OS=27.0` alone is insufficient. Verify both `xcrun simctl list runtimes --json` and `xcrun simctl runtime list`; conflicting builds must be resolved deliberately without deleting another worktree's runtimes. The local guard fails closed when it cannot verify pins or encounters a mismatch.
- **iPhone model**: use iPhone 17 hardware on the pinned runtime, preferably a dedicated per-worktree simulator's literal UDID. Custom simulator names are valid when their `deviceTypeIdentifier` and runtime match; never pick the first available iPhone.
- **Intentional repins**: a new beta, RC, or GA requires updating build assertions/cache identities and reviewing the complete baseline migration. Do not remove variants or loosen tolerances to make a toolchain migration pass. Existing documented sub-pixel tolerance remains limited to custom-font anti-aliasing, never structural changes.

Before recording, confirm the Xcode + runtime + device match CI's resolved trio. If local parity is unavailable, record and verify on the exact CI image, download and review the image changes, and document the outstanding local-validation gap. A recording pass is not a verification pass: rerun with recording disabled. Keep the app deployment minimum at **iOS 26.0**; use the guard's explicit `SUPER_IOS_COMPATIBILITY=1` build/build-for-testing path for older-runtime app compatibility, never for snapshot test execution or recording.

**Test on a dedicated, per-worktree simulator — not the shared booted one.** Create it with `xcrun simctl create "SB-<worktree-name>" "iPhone 17" com.apple.CoreSimulator.SimRuntime.iOS-27-0`, then address its returned UDID explicitly. Another session's install/launch must not collide with yours. **Delete only that simulator when the PR merges** (verify `MERGED`, then `simctl shutdown <id>` + `simctl delete <id>`); leave the worktree and branch in place per the worktree-discipline rule.
