...
- A `PreToolUse` hook (`.codex/hooks/block-outside-worktree.sh`, wired in `.codex/hooks.json`) backstops all of the above: it denies any Edit/Write/NotebookEdit target outside the active worktree. It's a safety net, not a substitute — the relative-path habit and the first-write sanity-check above are still the first line of defense.
