...
- **Active milestone**: M5 — Compaction (next up)
- **Last action**: M4 done — `ChatSession` + `ChatSessionStore` actors landed under `Packages/Chat/Sources/Chat/Orchestration/` along with `ChatEvent`. `ChatSession` owns one conversation's turn loop: save the user `MessageRecord` → loop { assemble `[LLMMessage]` from GRDB rows (assistant rows fold their `ToolCallRecord`s in as `.toolUse` blocks; tool-result rows look up their originating call to set `isError`) → drive `provider.stream(...)` with the currently-enabled tools → buffer text/thinking deltas in memory and persist the assistant `MessageRecord` only on `.messageComplete` (ADR-BB-003) → save one `ToolCallRecord(.pending)` per requested call → execute each via `ToolRegistry` and write a tool-result `MessageRecord` (role `.tool`) → loop }. Cancellation: `cancel()` cancels the session's `Task`; `CancellationError` from inside tool execution is re-thrown rather than treated as a tool failure, so the session terminates with `.error(.cancelled)` instead of marking the call `.failed`. `ChatSessionStore` is the singleton actor over `[ConversationID: ChatSession]` with get-or-create `session(for:)`, `cancel(for:)`, `runningConversations()`, and `shutdown()` semantics. Both actors take an injected `Clock` and `IDGenerator` (from Core's `Ambient/`) so tests use a `MonotonicClock` (auto-advances per `now()` so back-to-back rows have unique `createdAt` for `MessageRepository.fetchAll`'s ordering) and a `DeterministicIDGenerator` for stable assertions. **115 Chat tests across 16 suites pass** (97 prior + 9 `ChatSession` + 5 `ChatSessionStore` + 4 `ChatSession tool loop`). Core stays green (70 tests). `xcodebuild -scheme Super build` for iPhone 17 sim still succeeds.
- **Repo root**: `/Users/bwang/Development/Super/`
- **Next concrete sub-step (M5)**: build `ContextAssembler` + `Compactor` actor + `TokenEstimator` under `Packages/Chat/Sources/Chat/Orchestration/`. Hook `Compactor` into `ChatSession` so the turn loop calls `ContextAssembler` first and triggers compaction when over the auto-compact threshold (default 0.75 of model's `maxContextTokens`). Wire the `/compact` slash command in the composer. Settings keys (`autoCompactEnabled`, `autoCompactThreshold`) land in M9.
