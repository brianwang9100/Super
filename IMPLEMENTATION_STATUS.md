...
- **Active milestone**: M4 — Session orchestration (next up)
- **Last action**: M3 done — `OpenAICompatibleLLMProvider` landed in `Packages/Chat/Sources/Chat/LLM/`. Three files: `OpenAIWireTypes.swift` (private Encodable request shapes + Decodable stream-chunk shapes; snake_case ↔ camelCase via `JSONEncoder/Decoder` key strategy), `OpenAIStreamReducer.swift` (stateful struct turning `OpenAIStreamChunk`s into the normalized `LLMStreamEvent` sequence — handles messageStart-once, thinking↔text block transitions with our own monotonic block-index counter, tool-call argument fragment accumulation per OpenAI tool-index, and a terminal `messageComplete(usage:)` with captured token usage), `OpenAICompatibleLLMProvider.swift` (the public conformer; `init(id:displayName:model:baseURL:apiKey:http:)` plus a `ModelConfiguration` convenience init; `apiKey` is optional so unauthenticated local servers like MLX/Ollama/LM Studio just work; HTTP errors map 401/403 → `.unauthorized`, 429 → `.rateLimited`, other 4xx/5xx → `.providerError(code:message:)`; tool parameters project to JSON Schema with `bool` → `boolean` translation; tool-result `LLMMessage`s become per-result OpenAI `role: "tool"` rows). Recognizes both `reasoning_content` (DeepSeek) and `reasoning` (OpenAI o-series) field names as thinking deltas. Always sets `stream_options.include_usage: true` so we get real token counts. **86 Chat tests across 13 suites pass** (60 from M2 + 12 new `OpenAIStreamReducer` cases + 14 new `OpenAICompatibleLLMProvider` cases). Four checked-in SSE fixtures under `Packages/Chat/Tests/ChatTests/Fixtures/`: `openai-plain.txt`, `openai-reasoning.txt`, `openai-toolcall.txt`, `openai-reasoning-and-tools.txt`. Tests inject a `FakeHTTPClient` (in `Tests/ChatTests/LLM/Helpers/`) that replays fixture bytes — no real network ever. Provider tests also assert request URL/method/headers, body shape (model, messages, stream, temperature, stream_options, tools), `nil` apiKey omits Authorization, and that an SSE chunk delivered in 17 byte slices still produces a complete event stream (exercises the SSE parser's partial-frame handling). Test target gained `resources: [.copy("Fixtures")]` and `exclude: ["Database/__Snapshots__"]` to clear the long-standing snapshot file warning. Core stays green (70 tests). `xcodebuild -scheme Super build` for iPhone 17 sim still succeeds.
- **Repo root**: `/Users/bwang/Development/Super/`
- **Next concrete sub-step (M4)**: build `ChatSession` actor + `ChatSessionStore` actor under `Packages/Chat/Sources/Chat/Sessions/`. Turn loop: assemble messages from GRDB → stream from `LLMProvider` → write user + assistant messages + tool calls to GRDB → execute tool calls via `ToolRegistry` → loop until no tool calls. Persistence only on `.messageCompleted` (matches ADR-BB-003). Multiple sessions run concurrently in one store; cancelling one does not cancel siblings.
