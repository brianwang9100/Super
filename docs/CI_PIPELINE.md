...
`.github/workflows/argos.yml` adds a separate visual-testing job on PRs and main pushes: pinned iOS simulator capture through `npm test`, followed by an Argos CLI upload. It uses GitHub OIDC authentication for Argos and preserves capture evidence. Main requires both the `argos` visual review status and the `native-previews` capture/upload check, alongside the five existing checks. The current inventory is 37 Argos captures (21 composer, 14 Settings, two UIKit probes) and 584 legacy images. The iOS build workflow retains all four legacy package legs and the remaining Settings suite. Appearance light/dark/full-height light and data exporting light retain their legacy coverage because repeatability prevented their migration. Suite discovery excludes the removed composer suite and preserves the remaining Settings methods. External-fork PRs are blocked until their Argos upload path is enabled and verified. See [ARGOS_SETUP.md](ARGOS_SETUP.md) for setup, baseline seeding, and local commands.
