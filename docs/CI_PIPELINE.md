...
Capture guards, inventory validation, and package discovery still run for documentation-only PRs. The always-reporting `ios-test` and `native-previews` gates accept that exemption only with successful discovery and exactly skipped expected capture jobs. Unknown selection output, discovery failure, or unexpected skipped/failed/cancelled capture jobs fail. No workflow-level path filter or branch-protection change is involved. This avoids five macOS jobs per qualifying PR; it does not claim a proportional runtime saving from fewer images.

On full runs, each shard publishes image and manifest evidence. Exact inventory validation checks image identities, decoding, dimensions, hashes, and commit/run/attempt identity. Missing or failed shards cannot produce a partial green visual build. `ios-test` aggregates the four package comparison jobs; `native-previews` requires native and package comparisons and validates their complete combined inventory. Failures retain comparison evidence for inspection. CI refuses baseline recording, needs no Argos credentials or CLI, and runs the same comparisons for fork PRs without privileged upload authentication.
...
The rollback removed the external `argos` requirement after the replacement checks passed and Codex approved the revision. Live protection was rechecked on September 9, 2026: `build`, `lint`, `gitleaks`, `ios-test`, `swift-test`, and `native-previews` remain required with their GitHub Actions App bindings. This optimization changes no protection settings. YAML alone cannot change required checks; verify live protection before merging. Open PRs using the retired workflow need to update from the replacement revision.

Visual coverage follows [VISUAL_TESTING_POLICY.md](VISUAL_TESTING_POLICY.md): reuse representative layouts and preserve documented regression risks. The rollback restored 623 images; the subsequent [coverage consolidation](VISUAL_TESTING_POLICY.md#september-2026-consolidation) retires 46 with named visual and behavioral replacement evidence. Further reductions require their own audit; the historical shortlist is not permission to delete matrices. Git history cleanup and its monitor remain paused; no history rewrite is part of this rollback.
