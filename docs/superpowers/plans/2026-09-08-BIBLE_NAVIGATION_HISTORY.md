...
## Integration-test cleanup compatibility

Main PR #347 (`9ae71fd1`) consolidated duplicate schema checks and strengthened historical upgrade assertions, causing two test-file conflicts. The reviewed integration plan keeps main's schema-test consolidation and v8 upgrade survival coverage, the history feature's v12-to-v13 seeded migration test, and the complete v13 SQL schema snapshot. The pre-rebase head is retained in `codex/history-352-before-integration-test-rebase`.

The new v8 fixture initially failed with SQLite's missing `navigationHistoryJSON` column error because it inserted today's record into the historical table. Seed that reading row with explicit v8 SQL columns, then compare the migrated result to the current record to verify nil history and unchanged reading data. This retains main's highlight, bookmark, and note survival assertions without coupling the old schema to future record fields.

Validation: the legacy failure was reproduced before the fixture fix; all 866 Bible tests in 86 suites then passed (main's consolidation accounts for the count change). All 45 focused iOS database/schema/history tests in six suites passed on the registered pinned simulator. Production sources, UI fixtures, PNG baselines, inventory, and renderer settings remain byte-identical to the previously approved revision, so no baseline recording or additional visual changes are needed. Separate implementation review approved the resolution with no actionable findings; changed-file lint and diff checks passed. Renewed current-head CI/Codex approval remains required before merge.
