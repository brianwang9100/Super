...
### CI cold-start follow-up

Run 34269336484 passed native, Chat, Todo, all macOS suites, both app builds, lint and secret scanning. Bible and Core failed before compiling or running tests: `simctl bootstatus -b` exceeded its 300-second deadline while fresh simulator data-migration plugins were running; Core advanced through several plugins before the cutoff. Codex explicitly approved revision 357a66bb94. Raise only the bounded simulator-readiness deadline to 900 seconds to accommodate hosted-runner cold initialization; do not retry tests, ignore boot failures, replace devices, or relax image/check validation. Keep the existing 90-minute package-job deadline. Review the narrow change, rerun driver guards, and require the next complete CI invocation and current-revision Codex review before merge.
