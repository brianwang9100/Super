...
- [ ] Review and accept all 29 new PCC/privacy UI snapshots, then pass fresh normal CI against the complete references.
...
- The changed About privacy copy previously had only a light baseline. Three explicit dark/XXL fixtures complete its Vellum light/dark × default/XXL matrix. The strict PCC whitelist now contains exactly **29 additions** (22 registration/list + four composer + three About), making **301 Chat / 572 total PNGs** and **272 Chat snapshot tests** expected. All original references and attribution checks remain required; missing arbitrary About names still fail. A fresh complete migration and recording-off verification must pass before import.
