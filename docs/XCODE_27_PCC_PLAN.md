...
Status: rebased onto the Argos mainline, 2026-09-08. The 2026-09-06 execution evidence and checked phase items below are historical pre-Argos results, not approval of this revision. Argos now owns all image baselines; the old recording/import workflow and Git PNGs are retired.

### Current reconciliation and acceptance gates

- Preserve main's complete 623-capture inventory and all 29 PCC/privacy additions: 652 total (604 package, 48 native). The 22 Settings registration/list fixtures use the package exporter; four PCC composer fixtures and three additional About fixtures use native previews. The existing About light capture stays in place. No scenario was dropped.
- Shared pins: Xcode 27.0 beta 6 / `27A5252f`, iOS 27.0 / `24A5423a`, iPhone 17, XcodeGen 2.45.4. `Scripts/VisualTesting/simulator-pins.json` is canonical. Current local parity is established without any machine-wide mutation.
- All four local macOS package suites passed on the integrated source (Core 348, Bible 841, Chat 1151, Todo 87; 2427 total), both unsigned simulator app builds passed, and the focused mocked PCC simulator suite passed 19/19. Native discovery/export produced 48 valid PNGs; a stable final run is still required because the dependency-lock order changed during that preliminary run.
- Full local capture and real same-build coverage aggregation remain pending on the committed revision. The collector now enumerates all tests, reuses serialized capture results, runs complementary logic only, unions raw line hits, and fails closed on incomplete/mismatched evidence. Core 80% and applets 70% floors remain unchanged; percentages are not yet established by the reconciled run.
- New CI, current-revision Codex approval, Argos visual approval, and verified merge remain required. The historical CI links below do not satisfy those gates.
- Final-source iOS 26 Debug/Release startup smoke remains pending on the disposable CI runner; local iOS 26 runtime is absent. Real-device local/PCC behavior, entitlement-enabled signing, full persona/tools and distribution validation remain separate open release gates.

### Historical acceptance checklist (2026-09-06)

The following statuses are retained as historical evidence; use the current gates above for this revision.
...
- [x] Review and accept all 29 new PCC/privacy UI snapshots.
- [x] Pass fresh normal CI against the complete references.
- [ ] Finish final-source Release app startup validation on iOS 26.0; both Debug apps passed.
- [ ] Perform real-device iOS 26 UI/local-generation checks separately.
...
- Blocker follow-up: all four workstreams were delegated. The local-host check still reports macOS 26.3.1(a)/Xcode 26.4.1; upgrade/restart approval is pending. Main's eight intervening commits (`98e2e248`) are being integrated, preserving its UI changes and moving the strict 27 snapshot guard into the new canonical `.codex/hooks` location. Worktree simulator creation now reuses exact compiler and dual runtime-inventory checks; cleanup remains independent. New baselines arriving from main require fresh recording-off verification under 27.
- Read-only signing inventory found valid development/distribution identities and eight unexpired profiles, but **none contains the PCC entitlement**. Super has development/distribution profiles; SuperBible has a development profile only. One connected, paired iOS 27 phone has Developer Mode enabled; Apple Intelligence readiness and real PCC access are not established. Approval status remains unknown until an authenticated Account Holder checks both identifiers; missing local profiles are not proof of denial. No account changes, signing, device installation, or upload occurred.
- Follow-up automation adds one full simulator package execution for source-identical physical-line coverage, retaining raw reports, source hashes, and exact execution/skip inventory on success and failure. Floors remain Core 80% / applets 70%. Script fixtures pass, but fresh full-suite CI must establish actual compliance; previous macOS-only percentages are not retroactively treated as passing.
- The iOS 26 launch timeout now captures failure-only, owned-app process identity/stack, screenshot, filtered system launch events, and resource metrics. Completed startup cases are persisted before a later failure. The launch remains one 120-second attempt, diagnostics 15 seconds each, and global/job limits unchanged; diagnostics never convert failure to success. A fresh integrated-source smoke run is still required.
...
- PCC migration [34063231582](https://github.com/brianwang9100/Super/actions/runs/34063231582), source `2c7349e3`, passed identical **269-test/26-suite** inventories in all three phases and **269/269** tests in recording-off verification. Its **298 Chat PNGs** match every manifest hash and contain exactly **26 approved additions**: 22 registration/list states plus four full-name composer states. It also intentionally updates **32 existing images** (27 Settings + five no-model Chat screens). The five Chat-screen changes remove only the misleading zero-capacity footer meter; other pixels are identical. Composer default/XXL labels fit and unknown context is hidden. Visual review of all 27 existing Settings pairs found two new layout defects: the title-policy footer lacked horizontal insets, and one-line Apple subtitles hid essential readiness reasons (and even local identity at XXL). That candidate set was not imported; the corrected replacement below was required. Earlier run `34046385373` correctly refused a zero-test compile failure caused by an invalid test-only protocol-member assertion; `2c7349e3` fixed it, and no failed-run baselines were imported.
- `dfb54aa0` fixes both Settings defects: Apple subtitles wrap vertically while non-Apple rows retain their existing one-line behavior; the title-policy footer uses the section's 20-point inset. Existing unavailable/XXL/list snapshots exercise the affected layouts, and all 22 new registration/list states were reviewed. Parse and no-cache SwiftLint pass. Migration [34065298338](https://github.com/brianwang9100/Super/actions/runs/34065298338) was canceled while its Mac job was still queued, superseded by the completed About privacy test matrix below; no artifacts were imported.
- The changed About privacy copy previously had only a light baseline. Three explicit dark/XXL fixtures complete its Vellum light/dark × default/XXL matrix. Complete migration [34065513055](https://github.com/brianwang9100/Super/actions/runs/34065513055), source `7ebc9d39`, **passed recording-off verification with 272/272 tests and identical 26-suite inventories across all three phases**. Every artifact hash, original reference, exact toolchain/runtime/device, and all **29 allowed additions** were verified before import. Its **301 Chat / 572 repository PNGs** are now accepted. Compared with the first PCC candidate, 22 Settings images changed: 17 footer-inset/reflow deltas and five full Apple status/local-XXL wrapping deltas. All were visually reviewed, along with the three new About images; no new layout regression was found. The other 276 shared images are byte-identical to the already reviewed first candidate. Guarded import changed 32 existing workspace images and added 29, leaving database snapshots unchanged. Final normal iOS CI must still verify the complete accepted set against the later startup fix.
...
- Fresh package CI [34064798645](https://github.com/brianwang9100/Super/actions/runs/34064798645), source `1e9a6509`, passed all **2,246 tests** and the aggregate gate. Normal Swift/iOS workflows retain main/PR behavior and now support explicit manual verification; temporary migration-branch push triggers were removed in `a1f61836`.
- Accepted PCC/privacy baselines are committed in `ca643be8`. Final normal iOS CI [34067327916](https://github.com/brianwang9100/Super/actions/runs/34067327916) **passed on that revision**: Core **39 tests / 4 suites**, Chat **272 / 26**, Bible **195 / 25**, Todo **42 / 9** — **548 iOS tests**. All seven required PCC cases (two OS-27-only SDK error tests and five new startup/metadata regressions) have explicit passing log entries. All four snapshot legs verified iPhone 17 / runtime `24A5423a`; all six Mac legs asserted Xcode `27A5252f`. Both apps passed Debug simulator and unsigned Release/device builds, and both `build` / `ios-test` aggregate gates passed without retries. Its executable source matches the final successful package run at `664a861b`; the intervening commit changes only PNG references and this execution document.
...
- Final review found a separate bootstrap dependency: constructing every saved provider awaited PCC context metadata, so even a dormant PCC row could delay opening either app. Fix `664a861b` hydrates PCC identity with unresolved context immediately and defers status/metadata to explicit resolution or generation. Local initial metadata remains intact; unsupported systems still reject PCC without touching its status source. Final SDK 27 package run [34066215274](https://github.com/brianwang9100/Super/actions/runs/34066215274) passed **2,251 tests / 212 suites**: Core 342, Chat 1,114, Bible 711, Todo 84. All five new strict startup regression tests have individual passing log entries; all four jobs and the aggregate gate passed. Syntax/lint also pass.
...
- Follow-up iOS 26 smoke [34064599124](https://github.com/brianwang9100/Super/actions/runs/34064599124), source `1e9a6509`, **passed all four Super/SuperBible × Debug/Release startup checks** in 31m17s. Every binary reports `minos 26.0` / `sdk 27.0`; exact iOS 26.0 runtime `23A343`, dedicated per-configuration iPhone 17 simulators, and six non-zombie observations of each exact launched process were verified. All four screenshots were reviewed: Bible content renders in both configurations, Super Debug shows its branded shell, and Super Release shows the expected empty New chat with Apple Intelligence — Local only. This is process-startup evidence, not interactive readiness or generation proof. The first install took about 2m55s, explaining the earlier two-minute timeout. The ten-minute install cap remains bounded, with no automatic resets/retries; app-specific failure probes cannot target unowned simulators or turn failure into success.
- Final-source iOS 26 smoke [34066714230](https://github.com/brianwang9100/Super/actions/runs/34066714230), source `664a861b`, failed **before any app build**: the exact iOS 26.0 runtime imported, but the fresh simulator never completed `bootstatus` within the existing 600-second cap. Both boot logs were empty, so no narrower cause is established. No app install/launch occurred and the constructor fix was not exercised. A single fresh-runner retry [34067571621](https://github.com/brianwang9100/Super/actions/runs/34067571621), source `ca643be8` with identical executable source, used unchanged pins and all original bounds. Baseline/documentation-only changes do not affect the executable source.
- That final retry **verified both Debug apps on iOS 26.0**: successful build/install, exact launched processes, bounded liveness observations (Super six samples; SuperBible two samples spanning the required window), and reviewed screenshots showing Super's empty chat with Local only and SuperBible's reader. Both fresh simulators booted. Super Release also built with `minos 26.0` / `sdk 27.0` and installed, but `simctl launch` exceeded its unchanged 120-second limit before returning a PID; SuperBible Release was never attempted. Super Release emitted system warnings from process 47000, but without the required returned PID/liveness checks this is not a passing startup result. All three warning signatures also appear in the earlier successful Release run, so they do not establish whether this failure is simulator infrastructure or app behavior. The run failed after 25m57s; evidence was retained and no further retries, timeout increases, or source edits were made. **Final-source Release startup, interactive behavior, and real-device validation remain open.**
- Final local validation repeated all **118 safeguard tests** (70 migration/smoke/coverage + 48 snapshot-policy tests), `git diff --check`, and no-cache SwiftLint successfully. The normal lint configuration reports repository warnings; no lint configuration or coverage threshold was weakened.
- **Coverage gap:** the old heuristic silently skipped all package reports. The repaired SwiftPM JSON report's final run (`664a861b`) measures macOS unit-only package source lines as Core **63.88%** (2,510/3,929), Chat **48.11%** (11,543/23,992), Bible **40.64%** (6,194/15,242), Todo **30.63%** (998/3,258). These do not meet the repository's Core 80% / applet 70% policy. UIKit and OS-27-only runs are separate; no combined coverage is claimed. Missing-report failures are now enforced, while percentage enforcement was not previously wired and remains an explicit gap. No thresholds were reduced.
- A read-only coverage investigation confirmed that available simulator result bundles expose line-level coverage, but they cover different source revisions and cannot safely be combined with the package percentages. The package run retained aggregate/per-file counts rather than raw line identities, and successful simulator legs did not retain every result bundle. A trustworthy combined report requires same-revision source hashes and raw unit/simulator line data; adding percentages or treating a green aggregate gate as policy compliance would be incorrect.
...
- A read-only developer-portal access check could not proceed because the Mac was locked. Account eligibility and managed-entitlement approval remain unverified; no capability request, signing change, or upload was made.
- **Release blockers still open:** entitlement eligibility/approval for both bundles; updated profiles; signed archive/export and eligible iOS 27 device validation (including the full SuperBible persona/tools); real iOS 26 interaction/local-generation checks; final-source iOS 26 Release startup completion. Reviewed snapshots and normal iOS CI are complete. No TestFlight upload has been dispatched.

## Outcome and scope

Deliver in two sequential implementation phases within **one combined PR**, as requested on 2026-09-06:
...
### Final blocker workstreams

- Coverage: execute complete package suites on the pinned iOS simulator and retain source-identical coverage evidence; enforce Core 80% / applet 70% without combining unrelated percentages.
- iOS 26 compatibility: diagnose the Release launch acknowledgement timeout with bounded, app-scoped failure evidence, then repeat the four startup checks on the integrated revision. A diagnostic screenshot is not a passing startup check.
- PCC access: verify approved entitlements, profiles, and a suitable physical device before claiming signed generation/tool support. Account access or enrollment decisions remain explicit user gates.
- Local parity: inventory the host and establish exact Xcode/runtime/device parity after an explicitly authorized macOS upgrade/restart. CI validation continues independently while approval is pending.
- Integration: incorporate current main without reverting its UI or agent-infrastructure changes, rerun affected snapshots with recording disabled, and keep all work on `codex/xcode-27-pcc`. Do not enable merge while acceptance gates remain unresolved.
...
- [x] Keep existing package/snapshot discovery, both app build legs, required gate names, coverage thresholds, and docs-only/skip semantics intact.
...
- [x] Turn recording off and rerun the full screenshot suites successfully on fresh normal CI. Keep representative migration evidence for review.
- [ ] Rerun the full screenshot suites locally once the exact toolchain/runtime can be installed; CI success does not close the local-parity gap.
...
The hosted simulator build and reviewed snapshot/normal-CI evidence are complete. Remaining prerequisites are local toolchain parity, account/bundle entitlement access, signed distribution/device validation, complete final-source iOS 26 Release startup evidence, and the explicit coverage-policy gap. None requires raising the iOS deployment minimum to 27.
