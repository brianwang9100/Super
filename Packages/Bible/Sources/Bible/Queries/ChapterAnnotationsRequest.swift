...
/// deleted — including writes from outside the reader (the sheet's
...
/// matching `BibleAnnotationsByTargetRequest`. The reader only needs
/// presence per verse group (bubble on/off), so ordering is a stability
/// nicety, not a rendering contract.
