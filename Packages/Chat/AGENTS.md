...
- The provider is gated under `#if DEBUG` end-to-end (`LLMProviderKind.debug`, the provider class, the host bootstrap's register/seed call sites — today only `SuperOSAppBootstrap`; `SuperBibleAppBootstrap` will register the same provider once it gains Chat at SB-M1 — and the `SettingsViewModel` switch arm). It compiles out of Release entirely.
- The host bootstrap's `seedDebugModelIfNeeded` (today only `SuperOSAppBootstrap.seedDebugModelIfNeeded`) inserts a single `kind = .debug` `ModelConfigurationRecord` on first launch and marks it selected iff no other row is selected. So a fresh `xcrun simctl install` lands on the debug model by default; if you already have a real model wired, the debug entry just shows up as an alternative in the model picker.
