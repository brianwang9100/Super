...
public protocol ModelConfigurationRepository: Sendable {
    ...
    /// Insert `record` atomically, but only if the table has no other
    /// rows at the moment of the write. Returns `record` when inserted,
    /// `nil` when the table already had rows.
    ///
...
public struct GRDBModelConfigurationRepository: ModelConfigurationRepository {
    ...
    public func insertIfEmpty(
        _ record: ModelConfigurationRecord
