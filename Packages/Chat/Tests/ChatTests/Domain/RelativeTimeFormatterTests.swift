...
struct RelativeTimeFormatterTests {
    ...
    func fortyEightHours() {
        ...
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "2 days ago")
    }

    @Test("six days stays in days bucket")
    func sixDays() {
        let date = now.addingTimeInterval(-6 * 86_400)
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "6 days ago")
    }

    @Test("seven days renders 'Last week'")
    func sevenDays() {
        let date = now.addingTimeInterval(-7 * 86_400)
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "Last week")
    }
    ...
    func fourteenDays() {
        ...
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "2 weeks ago")
    }

    @Test("twenty-nine days stays in weeks bucket")
    func twentyNineDays() {
        let date = now.addingTimeInterval(-29 * 86_400)
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "4 weeks ago")
    }

    @Test("thirty days crosses into months bucket")
    func thirtyDays() {
        let date = now.addingTimeInterval(-30 * 86_400)
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "1 mo ago")
    }

    @Test("ninety days renders '3 mo ago'")
    func ninetyDays() {
        let date = now.addingTimeInterval(-90 * 86_400)
        #expect(RelativeTimeFormatter.format(date, now: now, calendar: calendar) == "3 mo ago")
    }
