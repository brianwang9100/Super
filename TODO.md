...
### CI gaps (still TODO)
- [ ] **Add a `Chat` test scheme** to `project.yml` so `xcodebuild test -scheme Chat` can run snapshot tests on iOS sim in CI. Currently the scheme is auto-created by Xcode locally but not generated by `xcodegen`, so CI can't run snapshot tests at all.
- [ ] **Codecov integration** — wire `codecov-action@v4` into the swift-test workflow once the iOS scheme above lands. Configure thresholds (Core ≥ 80%, Chat ≥ 70%) per `AGENTS.md`.
