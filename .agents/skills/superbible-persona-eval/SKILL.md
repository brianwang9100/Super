...
- For a real shard, state the selected models, cases, iterations, concurrency, judge, and reasoning effort before running. Pass each selection explicitly with `--model`, `--case`, `--iterations`, `--concurrency`, `--judge`, and `--reasoning-effort` as needed.
...
Use the developer's existing Codex login. Never request, set, or expose an API key.
